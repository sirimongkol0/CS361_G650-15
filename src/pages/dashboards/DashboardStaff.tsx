    ...
    <div className="p-6 max-w-screen-xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <select className="input py-2 text-sm" style={{ width: "auto" }} value={year} onChange={(e) => setYear(e.target.value)}>
      ...
      {expiringDocs.length > 0 && (
        <div className="flex items-center gap-3 p-4 rounded-xl mb-5" style={{ background: "#FEF3C7", border: "1px solid #FDE68A" }}>
          ...
          <div className="flex-1 text-sm" style={{ color: "#92400E" }}>
            <span className="font-bold">⚠ {expiringDocs.length} ข้อตกลงใกล้หมดอายุ</span>
            {expiringDocs.map((d, i) => (
              <span key={i}> — {d.title} เหลืออีก {d.days} วัน</span>
            ))}
          </div>
          <Link to="/documents" className="btn text-xs py-1.5 px-3" style={{ background: "#B45309", color: "#fff" }}>ดูรายการ</Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-6 gap-3 mb-6">
        {[
          { icon: Building2, label: "Stakeholder", value: "48", color: "#8B1538", bg: "#F5D6DE", to: "/stakeholders" },
          { icon: FileText, label: "MoU / MoA", value: "23", color: "#B45309", bg: "#FEF3C7", to: "/documents" },
          { icon: CalendarDays, label: "กิจกรรม", value: "156", color: "#1D4ED8", bg: "#DBEAFE", to: "/activities" },
          { icon: GraduationCap, label: "นักศึกษา", value: "32", color: "#15803D", bg: "#DCFCE7", to: "/exchange" },
          { icon: MessageSquare, label: "Feedback", value: "128", color: "#7C3AED", bg: "#EDE9FE", to: "/feedback" },
          { icon: AlertTriangle, label: "ใกล้หมดอายุ", value: "2", color: "#B45309", bg: "#FEF3C7", to: "/documents" },
        ].map((s) => (
          <Link key={s.label} to={s.to} className="stat-card block" style={{ textDecoration: "none", padding: "16px 14px" }}>
      ...
      <div className="grid gap-5 mb-5" style={{ gridTemplateColumns: "1fr 380px" }}>
        ...
        <div className="content-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/documents" className="text-xs font-semibold flex items-center gap-1" style={{ color: "#8B1538" }}>
            ...
            {expiringDocs.map((d, i) => (
              <div key={i} className="p-3.5 rounded-xl" style={{ background: "#FEF3C7", border: "1px solid #FDE68A" }}>
                <div className="flex items-start justify-between">
                  ...
                  <span className="badge badge-yellow">{d.days} วัน</span>
                </div>
                <div className="text-xs mt-1" style={{ color: "#B45309" }}>หมดอายุ {d.expire}</div>
                <div className="flex gap-2 mt-2.5">
                  <Link to="/documents/1" className="btn text-xs py-1.5 flex-1 justify-center" style={{ background: "#8B1538", color: "#fff" }}>ดูเอกสาร</Link>
          ...
          <table className="w-full">
            ...
            <tbody>
              {recentActivities.map((a, i) => (
                <tr key={i} className="table-row-hover border-b" style={{ borderColor: "#F3F4F6" }}>
                  ...
                  <td className="px-5 py-3.5">
                    <Link to="/activities/1" className="btn btn-ghost p-1.5 text-xs">ดูรายละเอียด</Link>
                  </td>
                </tr>
              ))}
            </tbody>
